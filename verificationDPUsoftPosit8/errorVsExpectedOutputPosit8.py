...
with open(input_file, "r") as f, open(debug_output, "w") as log:
    ...
    for line_num, line in enumerate(f):
        ...
        try:
            ...
            a = [Posit8.from_bits(int(parts[i], 16)) for i in range(4)]
            b = [Posit8.from_bits(int(parts[i], 16)) for i in range(4, 8)]
            c = Posit8.from_bits(int(parts[8], 16))
            sim = Posit8.from_bits(int(parts[9], 16))
...
plt.title("Absolute Error vs Expected Output (SoftPosit8 DPU)")
plt.xlabel("Expected Output (Golden Value)")
plt.ylabel("Absolute Error")
plt.grid(True)
plt.tight_layout()
plt.show()
plt.savefig("softposit8_dpu_error_plot.png", dpi=300)

print("\n✅ Plot saved as 'softposit8_dpu_error_plot.png'")
print("📄 Debug log saved as 'posit8_dpu_debug_log.txt'")
