...
debug_output = "float16_dpu_debug_log.txt"
...
with open(input_file, "r") as f, open(debug_output, "w") as log:
    ...
    for line_num, line in enumerate(f):
        ...
        try:
            # Decode operands from hex as Float16 (16-bit words)
            a = [Float16.from_bits(int(parts[i], 16)) for i in range(4)]
            b = [Float16.from_bits(int(parts[i], 16)) for i in range(4, 8)]
            c = Float16.from_bits(int(parts[8], 16))
            sim = Float16.from_bits(int(parts[9], 16))
...
plt.scatter(golden_results, abs_errors, s=4, alpha=0.6, color = 'green' )
plt.title("Absolute Error vs Expected Output (Float16 DPU)")
plt.xlabel("Golden Result (Float16)")
plt.ylabel("Absolute Error")
plt.grid(True)
plt.tight_layout()
plt.show()
plt.savefig("float16_dot_product_error_plot.png", dpi=300)

print("\n✅ Plot saved as 'float16_dpu_error_plot.png'")
print("📄 Debug log saved as 'float16_dpu_debug_log.txt'")
