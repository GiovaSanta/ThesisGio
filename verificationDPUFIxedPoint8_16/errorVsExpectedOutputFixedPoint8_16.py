...
# Files
input_file = "FixedPoint8_16_dpu_output_from_HDL_circuit_.txt"
debug_output = "FixedPoint8_16_dpu_debug_log.txt"
...
with open(input_file, "r") as f, open(debug_output, "w") as log:
    ...
    for line_num, line in enumerate(f):
        ...
        try:
            ...
            a = [Fxp(signed=True, n_word=8, n_frac=5) for _ in range(4)]
            b = [Fxp(signed=True, n_word=8, n_frac=5) for _ in range(4)]
            for i in range(4):
                a[i].set_val("0x" + parts[i], raw=True)
                b[i].set_val("0x" + parts[i+4], raw=True)

            c = Fxp(signed=True, n_word=16, n_frac=10)
            c.set_val("0x" + parts[8], raw=True)

            sim = Fxp(signed=True, n_word=16, n_frac=10)
...
plt.title("Absolute Error vs Expected Output (FixedPoint8_16 DPU)")
plt.xlabel("Expected Output (Golden Value)")
plt.ylabel("Absolute Error")
plt.grid(True)
plt.tight_layout()
plt.show()
plt.savefig("FixedPoint8_16_dpu_error_plot.png", dpi=300)

print("\n✅ Plot saved as 'FixedPoint8_16_dpu_error_plot.png'")
print("📄 Debug log saved as 'FixedPoint8_16_dpu_debug_log.txt'")
